 ...
 * @example
 * ```ts
 * getInitials('John Doe') // 返回 'JD'
 * getInitials('Mary Jane Watson') // 返回 'MJW'
 * getInitials('') // 返回 ''
 * ```
 * 
 * @param fullName - 需要提取首字母的完整姓名
 * @returns 由大写首字母组成的字符串
 */
...
export function getCreditCardBrandName(number: string) {
  for (const [type, regex] of Object.entries(creditCardRegex)) {
...
export function camelCaseToTitleCase(camelCaseStr: string) {
  const titleCaseStr = camelCaseStr
    .replace(camelCaseRegex, ' $1') // Insert space before uppercase letters
    .replace(firstCharRegex, (char) => char.toUpperCase()); // Capitalize the first letter
